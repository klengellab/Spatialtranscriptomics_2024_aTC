...
eigengene_network <- function(Expr, module, metadata){
  ...
  pdf(paste0(output_path, "eigengene_network_signed.pdf"))
...
module_trait <- function(Expr, metadata){
  ...
  pdf(paste0(output_path, "module_trait_relationship_raw_signed.pdf"), width = 12, height = 10)
  par(mar = c(4,8,2,2))
...
MEs0 <- module_trait(data, traitData) # I have now the correct data and want to look further into the following modules
# darkgrey, lightgreen, white, royalblue, orange

# Relate modules to phenotype data with FDR adjustment
module_trait <- function(Expr, metadata){
  # Visualization of modules with the phenotype data
  nGenes = ncol(Expr)
  nSamples = nrow(Expr)
  ...
  modTraitCor = stats::cor(MEs0, metadata, use = "p", method = "pearson")
  modTraitP = corPvalueStudent(modTraitCor, nSamples)
  modTraitP <- p.adjust(modTraitP, method = "BH", n = 87)
  textMatrix = paste(signif(modTraitCor, 2), "\n(",
                     signif(modTraitP, 1), ")", se = "")
  dim(textMatrix) = dim(modTraitCor)
  pdf(paste0(output_path, "module_trait_relationship_raw_signed_corrected_p.pdf"), width = 12, height = 10)
  par(mar = c(4,8,2,2))
  labeledHeatmap(Matrix = modTraitCor, xLabels = names(metadata), 
                 yLabels = names(MEs0), ySymbols = names(MEs0),
                 colorLabels = F, colors = blueWhiteRed(50),
